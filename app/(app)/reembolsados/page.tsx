...
const PAGE_SIZE = 10;
const REFUNDED_STATUSES = ['refunded', 'refunded_payment', 'refunded_payment_gateway'] as const;
...
export default async function RefundedSalesPage({ searchParams }: RefundedSalesPageProps) {
  ...
  const { items, total } = await listSales(page, PAGE_SIZE, REFUNDED_STATUSES, query);
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold tracking-tight text-slate-900">Vendas reembolsadas</h1>
        <p className="text-sm text-slate-500">
          Lista completa das vendas com pagamento reembolsado armazenadas no Supabase a partir das sincronizações
          ...
          <form method="get" className="flex w-full flex-col gap-2 md:w-auto md:flex-row">
            ...
            <div className="flex items-center gap-2 md:justify-end">
              ...
              <Button variant="ghost" asChild>
                <Link href="/reembolsados">Limpar</Link>
              </Button>
            </div>
          </form>
        </CardHeader>
            ...
            <div className="flex items-center gap-2">
              {prevPage ? (
                <Button variant="outline" size="sm" asChild>
                  <Link href={`/reembolsados?page=${prevPage}${pageQuerySuffix}`}>
                    Página anterior
                  </Link>
                </Button>
              ) : (
              ...
              {nextPage ? (
                <Button variant="outline" size="sm" asChild>
                  <Link href={`/reembolsados?page=${nextPage}${pageQuerySuffix}`}>
                    Próxima página
                  </Link>
                </Button>
              ) : (
