...
interface RefusedSalesPageProps {
  readonly searchParams?: Record<string, string | string[]>;
}

const PAGE_SIZE = 10;
const REFUSED_STATUSES = ['refused', 'refused_payment', 'refused_payment_gateway'] as const;
...
export default async function RefusedSalesPage({ searchParams }: RefusedSalesPageProps) {
  ...
  const { items, total } = await listSales(page, PAGE_SIZE, REFUSED_STATUSES, query);
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-2">
        <h1 className="text-3xl font-semibold tracking-tight text-slate-900">Vendas recusadas</h1>
        <p className="text-sm text-slate-500">
          Lista completa das vendas com pagamento recusado armazenadas no Supabase a partir das sincronizações
          ...
          <form method="get" className="flex w-full flex-col gap-2 md:w-auto md:flex-row">
            ...
            <div className="flex items-center gap-2 md:justify-end">
              ...
              <Button variant="ghost" asChild>
                <Link href="/recusados">Limpar</Link>
              </Button>
            </div>
          </form>
        </CardHeader>
            ...
            <div className="flex items-center gap-2">
              {prevPage ? (
                <Button variant="outline" size="sm" asChild>
                  <Link href={`/recusados?page=${prevPage}${pageQuerySuffix}`}>
                    Página anterior
                  </Link>
                </Button>
              ) : (
              ...
              {nextPage ? (
                <Button variant="outline" size="sm" asChild>
                  <Link href={`/recusados?page=${nextPage}${pageQuerySuffix}`}>
                    Próxima página
                  </Link>
                </Button>
              ) : (
