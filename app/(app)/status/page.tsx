...
export default function StatusPage() {
  ...
  const loadHealth = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || (payload as { ok?: boolean }).ok === false || !isHealthResponse(payload)) {
...
function isHealthResponse(value: unknown): value is HealthResponse {
  ...
  if (value.db !== 'ok' && value.db !== 'unknown') {
    return false;
  }

  if (value.storage !== 'ok' && value.storage !== 'unknown') {
    return false;
  }
  ...
  if (typeof value.failedEventsCount !== 'number' || typeof value.jobsPending !== 'number') {
    return false;
  }
