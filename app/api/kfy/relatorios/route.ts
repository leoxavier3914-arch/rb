...
const dimensionHandlers = {
  day: (row: any) => format(new Date(row.approved_at ?? row.created_at), "yyyy-MM-dd"),
  product: (row: any, context: { productMap: Map<number, string> }) =>
    context.productMap.get(row.product_id) ?? "Produto desconhecido",
  method: (row: any) => row.payment_method ?? "indefinido",
  status: (row: any) => row.status ?? "indefinido",
} as const;

type DimensionKey = keyof typeof dimensionHandlers;

const metricHandlers = {
  gross: (row: any) => Number(row.gross_cents ?? 0),
  net: (row: any) => Number(row.net_cents ?? 0),
  commission: (row: any) => Number(row.commission_cents ?? 0),
  orders: () => 1,
} as const;
