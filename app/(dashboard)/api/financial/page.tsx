...
export default async function FinancialPage() {
  ...
  const percentStatEntries: PercentStatEntry[] = stats
    ? [
        ...Object.entries(PERCENT_STAT_LABELS)
          .map(([key, meta]) => {
            const value = stats?.[key];
            if (value === null || value === undefined) return null;
            return { key, label: meta.label, helper: meta.helper, value } satisfies PercentStatEntry;
          })
          .filter((entry): entry is PercentStatEntry => entry !== null),
        ...Object.entries(stats)
          ...
          .map(([key, value]) => ({ key, label: humanizeStatKey(key), value } satisfies PercentStatEntry)),
      ]
    : [];
