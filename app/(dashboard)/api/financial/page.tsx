...
async function fetchFinancialSummaryFromApi(): Promise<FinancialSummary> {
  ...
  const url = `${protocol}://${host}/api/financial`;
  const response = await fetch(url, { cache: "no-store" });

  if (!response.ok) {
    throw new Error(`Falha ao consultar /api/financial: ${response.status}`);
  }
...
export default async function FinancialPage() {
  ...
  try {
    ...
    console.error("Erro ao consultar dados financeiros na rota interna /api/financial", err);
    error =
