...
const SALES_API_BASE_PATH = "/api/kiwify/sales" as const;
...
const normalizeSale = (payload: Record<string, unknown>): NormalizedSale => {
  ...
  const paymentMethod = pickString(payload, PAYMENT_METHOD_DISPLAY_PATHS);
...
const fetchSales = async ({ status, startDate, endDate, productId }: FetchSalesParams) => {
  ...
  const url = new URL(SALES_API_BASE_PATH, baseUrl);
...
const fetchSummary = async ({
  ...
  const baseUrl = buildBaseUrl();
  const url = new URL(`${SALES_API_BASE_PATH}/summary`, baseUrl);
...
export default async function SalesPage({ searchParams }: SalesPageProps) {
  ...
  const initialPage = Number(Array.isArray(pageParam) ? pageParam[0] : pageParam);
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Consulte vendas registradas via API oficial da Kiwify com filtros por status, período e busca por produtos ou
              compradores.
            </p>
          </div>
        </div>
      ...
      {error ? (
        <div className="rounded-3xl border border-red-500/40 bg-red-500/10 p-6 text-sm text-red-100">{error}</div>
      ) : (
...
export type { SalesSummaryResponse as SalesSummary };
