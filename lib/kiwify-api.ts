...
const OFFER_PRICE_CENTS_PATHS = [
  ...
  "default_price.priceCents",
  "default_price.amount_cents",
  "default_price.amountCents",
  "default_price.value_cents",
  "default_price.valueCents",
  "defaultPriceCents",
  "defaultPrice.price_cents",
  "defaultPrice.priceCents",
  "defaultPrice.amount_cents",
  "defaultPrice.amountCents",
  "defaultPrice.value_cents",
  "defaultPrice.valueCents",
  "current_price_cents",
  "currentPriceCents",
  "pricing.price_cents",
...
export async function getKiwifyProducts(): Promise<ProductsResult> {
  ...
  const products = rawItems
    ...
    .map((record) => {
      const source: UnknownRecord = isRecord(record.attributes)
        ? { ...record, ...(record.attributes as UnknownRecord) }
        : record;
