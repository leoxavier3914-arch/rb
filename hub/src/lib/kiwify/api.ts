...
export async function syncSalesFromKiwify(): Promise<SyncSalesResult> {
  ...
  while (true) {
    ...
    const upsertItems = Array.from(result.items, (item) => ({ ...item } satisfies SalesInsert));
    const { error } = await supabase.from("sales").upsert(upsertItems, { onConflict: "id" });
    if (error) {
